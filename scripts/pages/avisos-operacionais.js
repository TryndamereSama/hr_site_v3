// MC1 HUB — Avisos Operacionais (Placeholder)
import { t } from '../i18n.js';

export function renderAvisosOperacionais(container) {
  container.innerHTML = `
    <section class="section-sm" style="background: var(--color-surface-container-low);">
      <div class="container">
        <nav class="breadcrumb" aria-label="Caminho de navegação">
          ...
          <svg width="14" height="14"><use href="#icon-chevron-right"/></svg>
          <span>${t('nav.operacional.avisos')}</span>
        </nav>
        <div class="page-header" style="padding-top:var(--space-6)">
          ...
          <h1>${t('nav.operacional.avisos')}</h1>
          <p>${t('nav.operacional.avisos.sub')}</p>
        </div>
      </div>
    </section>
    ...
    <section class="section">
      <div class="container">
        ...
        " data-reveal>
          <div class="icon-wrap icon-wrap-xl" style="background: linear-gradient(135deg, #ea580c, #f97316); color: white; margin-bottom: var(--space-6);">
            <svg width="36" height="36"><use href="#icon-newspaper"/></svg>
          </div>
          ...
          <p class="body-lg text-muted" style="max-width: 38ch; margin-bottom: var(--space-8)">
            ${t('nav.operacional.avisos.coming_soon_desc')}
          </p>
